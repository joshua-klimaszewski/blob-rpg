...
type QuestTab = 'accepted' | 'new' | 'completed';
...
export function GuildScreen() {
  ...
  const [activeTab, setActiveTab] = useState<QuestTab>('accepted');
  ...
  return (
    <div className="flex flex-col items-center gap-4 p-6">
      ...
      {/* Tab Navigation */}
      <div className="flex w-full max-w-half border-2 border-ink">
        <button
          onClick={() => setActiveTab('accepted')}
          className={`flex-1 min-h-touch px-3 py-2 text-sm font-bold border-r-2 border-ink ${
            activeTab === 'accepted' ? 'bg-ink text-paper' : 'bg-paper text-ink active:bg-gray-100'
          }`}
        >
          Accepted {unclaimedQuests.length > 0 && `(${unclaimedQuests.length})`}
        </button>
        <button
          onClick={() => setActiveTab('new')}
          className={`flex-1 min-h-touch px-3 py-2 text-sm font-bold border-r-2 border-ink ${
            activeTab === 'new' ? 'bg-ink text-paper' : 'bg-paper text-ink active:bg-gray-100'
          }`}
        >
          New {unacceptedQuests.length > 0 && `(${unacceptedQuests.length})`}
        </button>
        <button
          onClick={() => setActiveTab('completed')}
          className={`flex-1 min-h-touch px-3 py-2 text-sm font-bold ${
            activeTab === 'completed' ? 'bg-ink text-paper' : 'bg-paper text-ink active:bg-gray-100'
          }`}
        >
          Completed {claimedQuests.length > 0 && `(${claimedQuests.length})`}
        </button>
      </div>
          ...
          <div className="flex flex-col gap-2">
            {unclaimedQuests.length === 0 ? (
              <div className="text-sm text-gray-400 text-center py-8">
                No active quests. Check the New tab!
              </div>
            ) : (
          ...
          <div className="flex flex-col gap-2">
            {unacceptedQuests.length >= 2 && (
              <button
                ...
                className="border-2 border-ink px-3 py-2 text-sm font-bold active:bg-ink active:text-paper"
              >
                Accept All ({unacceptedQuests.length})
              </button>
            )}
            {unacceptedQuests.length === 0 ? (
              <div className="text-sm text-gray-400 text-center py-8">
                No new quests available. Progress further in the dungeon!
              </div>
            ) : (
              unacceptedQuests.map((quest) => (
                <div
                  key={quest.id}
                  className="border-2 border-ink px-3 py-2 text-sm"
                >
